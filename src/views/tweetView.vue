...
import {fetchTweetDetails} from "../api/apiTweet.ts";
    ...
    <div class="flex justify-center pt-24">
      <div class="w-2/5 h-auto pt-3 pb-4 px-5 bg-homeCard bg-opacity-5 rounded-[10px]">
        <div class="w-full flex flex-col">
          ...
          <hr class="w-full my-3 border-white border-opacity-10"/>
          <div class="flex mt-3">
            <div class="w-[40px] h-[40px]">
              <img class="w-[40px] h-[40px] rounded-full mr-4" v-if="tweetDetail.user.avatar_url" :src="tweetDetail.user.avatar_url" alt="Profile Picture">
              <div v-if="!tweetDetail.user.avatar_url" class="w-[40px] h-[40px] mr-4 bg-white rounded-full"></div>
            </div>

            <div class="flex flex-col flex-grow pl-4">
              <div class="flex items-center">
                <h2 class="text-white font-medium">{{ tweetDetail.user.full_name}}</h2>
                <p class="text-gray-400 text-xs pl-4">{{ tweetDetail.created_at }}</p>
              </div>
              <p class="text-white text-sm mt-2">{{ tweetDetail.body }}</p>
              ...
              <div class="flex flex-row text-sm mt-4">
                <div class="flex flex-row items-center pr-14">
                  ...
                  <p class="pl-2 text-white">{{ tweetDetail.likes_count}}</p>
                </div>
                ...
                <div class="flex flex-row items-center">
                  ...
                  <p class="pl-2 text-white">{{ tweetDetail.comments_count}}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="pt-20 w-full flex">
            ...
            <div class="flex flex-col flex-grow pl-4">
              <textarea placeholder="Kommentiere den Beitrag" class="bg-transparent text-base text-white outline-none w-full" maxlength="200"></textarea>
              <div class="mt-4 flex justify-end items-center">
                <button class="w-[154px] h-10 rounded-md bg-homeCard text-black font-semibold">Kommentieren</button>
              </div>
            </div>
          </div>
